...
class UserProfile(models.Model):
    ...
    matches = models.OneToManyField(UserProfile, blank=True)
...
class SportingRequest(models.Model):
    ...
    sport = models.CharField(max_length=128)
    duration = models.DurationField(blank=True) # Blank since it can be optional
    status = models.BooleanField(default=True) # To see if request has been completed. True = Request open, False = Request closed
    start_user = models.ForeignKey(UserProfile, on_delete = models.CASCADE)
    involved_users = models.OneToManyField(UserProfile, blank=True)
