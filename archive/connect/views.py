...
def rate(request, ratedUserID = 0):
    ...
    try:
        ...
        try:
            ...
            userToRate = UserProfile.objects.all() \
...
def getMatches(currentProfile):
    matches1 = [ UserProfile.objects.get(id=e['user2']).user for e in Match.objects.filter(user1 = currentProfile).values('user2') ]
    matches2 = [ UserProfile.objects.get(id=e['user1']).user for e in Match.objects.filter(user2 = currentProfile).values('user1') ]
